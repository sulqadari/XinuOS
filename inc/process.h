...
#define PROCESS_SLEEP                   4   // sleeping (waiting for timer)
...
// must be odd to make processEntry 4N bytes (whatever that means...)
...
extern uint32_t currentProcessId;       // stores ID of currently running process

// inline code to check process ID (assumes)
#define IS_BAD_PROCESS_ID(x)    (   ((uint32_t)(x) < 0)\
